...
local function extendRent(cell, daysToRent, player)
    ...
    addOrRemoveWithMessage("gold_001", totalCost, true)
    totalCost = totalCost + I.ZS_DataManager.getData("StayCost_" .. sanitizeString(cell.name))
...
local function checkOutPlayer(cell, player)
    ...
    if noteId and playerHasItemCount(noteId, 1) == 1 then
        addOrRemoveWithMessage(noteId, 1, true)
    end
    local keyId = getRoomKeyID(cell)
    if keyId and playerHasItemCount(keyId, 1) == 1 then
...
local function npcActivation(npc, actor)
    ...
    if class:lower() == "publican" then
        ...
        if not innBed then
            world.mwscript.getGlobalVariables(actor)["IH_ModEnabled"] = 0
        else
            world.mwscript.getGlobalVariables(actor)["IH_ModEnabled"] = 1
            if not innDoor then
                world.mwscript.getGlobalVariables(actor)["IH_HasRoomNotJustBed"] = 0
            else
                world.mwscript.getGlobalVariables(actor)["IH_HasRoomNotJustBed"] = 1
            end
        end
        local goldPerNight = I.ZS_InnOverhaul_Settings.getSetting("goldPerNight")
        local goldPerWeek = I.ZS_InnOverhaul_Settings.getSetting("goldPerWeek")
        world.mwscript.getGlobalVariables(actor)["IH_GoldCost_1"] = goldPerNight
        world.mwscript.getGlobalVariables(actor)["IH_GoldCost_2"] = goldPerNight * 2
        world.mwscript.getGlobalVariables(actor)["IH_GoldCost_7"] = goldPerWeek

        world.mwscript.getGlobalVariables(actor)["IH_HasGold_1"] = playerHasItemCount("gold_001", goldPerNight)
        world.mwscript.getGlobalVariables(actor)["IH_HasGold_2"] = playerHasItemCount("gold_001", goldPerNight * 2)
        world.mwscript.getGlobalVariables(actor)["IH_HasGold_7"] = playerHasItemCount("gold_001", goldPerWeek)

        if world.mwscript.getGlobalVariables(actor)["IHS_1"] == 1 then
            local daysPassed, currentHour = getGameTimeData()
            local checkoutDay = I.ZS_DataManager.getData("CheckOutDay_" .. sanitizeString(npc.cell.name))
            world.mwscript.getGlobalVariables(actor)["IH_NightsRemaining"] = checkoutDay - daysPassed
            local keyID = getRoomKeyID(npc.cell)
            if keyID then
                print("Has key? " .. keyID)
                world.mwscript.getGlobalVariables(actor)["IH_HasKey"] = playerHasItemCount(keyID, 1)
            end
        end
...
local function RestStart(data)
    ...
    if returnstage ~= -1 and sleeperstage ~= -1 then
        types.Player.quests(world.players[1])["TR_dbAttack"].stage = returnstage
        types.Player.quests(world.players[1])["A2_2_6thHouse"].stage = sleeperstage
    end
    ...
    if bed and not isWait and isInnBed(bed) and bedRented(bed) and safeSleep then
        ...
        if types.Player.quests(world.players[1])["TR_dbAttack"].stage ~= 51 then
            returnstage = types.Player.quests(world.players[1])["TR_dbAttack"].stage
            sleeperstage = types.Player.quests(world.players[1])["A2_2_6thHouse"].stage
        end
        types.Player.quests(world.players[1])["TR_dbAttack"].stage = 51
        types.Player.quests(world.players[1])["A2_2_6thHouse"].stage = -10
